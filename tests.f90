...
MODULE o_lagrange
	...
	use divide_grid
	implicit none 	
	integer :: AllocateStatus, i, j, n, k					! Indexes and statuses	
...
contains
	...
	SUBROUTINE o_alloc_data
		...
		call o_alloc_check(AllocateStatus); flags = -2
	...
	SUBROUTINE o_set_default_constants
		...
		TIMESTEPS = 720			!count of TIMESTEPS in model
		DT = 100			!timestep
	...
	SUBROUTINE test1()
		...
		integer req, stats(MPI_STATUS_SIZE)
		if (rank /= MAIN_PROC) THEN
			call MPI_SEND(sbuf,1000000,MPI_DOUBLE_PRECISION, MAIN_PROC, 12, MY_COMM, req,ierr)
		else
			call MPI_RECV(rbuf,1000000, MPI_DOUBLE_PRECISION, MPI_ANY_SOURCE, 12,MY_COMM,stats,ierr)
		end if
	END SUBROUTINE
	...
	SUBROUTINE test2()
		...
		do j=0,count_proc-1
			if (j /= rank) THEN
				call MPI_SEND(sbuf,N_TEST,MPI_DOUBLE_PRECISION, j, 12, MY_COMM, req,ierr)
				call MPI_RECV(rbuf,N_TEST,MPI_DOUBLE_PRECISION, j,12,MY_COMM,stats,ierr)
			end if
		end do
	END SUBROUTINE

	SUBROUTINE test3(step)
		integer, intent(in) :: step
		integer, parameter :: N_TEST = 1000 
		logical flag
		real (8) :: buffer(N_TEST)
		real (8) :: recv_buffer(N_TEST)
		integer ierr
		integer req, stats(MPI_STATUS_SIZE)
		do n=1, N_P
			if ((rank == 3) .and. ((step == 227) .or. (step == 277))) THEN
				call MPI_SEND(buffer,N_TEST, MPI_DOUBLE_PRECISION, 4,11,MY_COMM,req,ierr)
			end if
		end do
		call MPI_BARRIER(MY_COMM, ierr)
		flag = .true.
		do while (flag .eqv. .true.)
			call MPI_IRECV(recv_buffer, N_TEST, MPI_DOUBLE_PRECISION, MPI_ANY_SOURCE, 11, MY_COMM, req, ierr)
			call MPI_TEST(req, flag, stats, ierr)
			if (flag .eqv. .true.) THEN
				!flags(id) = 1
			else
		...
		if (rank == MAIN_PROC) THEN
			do j=1, N_P
				if (flags(j) == 1 .or. flags(j) == -1 .or. flags(j) == -2) THEN
					...
					print *, "recv"
					call MPI_RECV(rbuf, arr_count*2+1, MPI_DOUBLE_PRECISION, MPI_ANY_SOURCE, 12,MY_COMM,stats,ierr)
					print *, "got"
				end if
		...
		do j = 1, N_P
			if (flags(j) == 1 .or. flags(j) == -1 .or. flags(j) == -2) THEN
				if (rank /= MAIN_PROC) THEN
					...
					sbuf(1) = dble(j)
					print *, "send"
					call MPI_SEND(sbuf,arr_count*2+1,MPI_DOUBLE_PRECISION, MAIN_PROC, 12, MY_COMM, req,ierr)
				end if
			end if
		end do
	END SUBROUTINE
