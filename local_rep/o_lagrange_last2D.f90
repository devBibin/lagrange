...
contains
	...
	SUBROUTINE set_default_constants
		N_P = 500000	 		!number of particles
		MAIN_PROC = 0			!index of main process, which gather info
		MY_COMM = comm_local		!communicator of module
		TIMESTEPS = 181			!count of TIMESTEPS in model
		DT = 10000			!timestep
		DEEP = 1			!deep of slice
		FREQUENCY_FOR_FILE = 181 	!write to file once per 10 iterations
	...
	SUBROUTINE o_set_particles_coordinates
		...
		do i=1,N_P
			...
			if (flags(i) == 1) THEN
				...
				x1_p_track(i,0) = x1_p(i)
				x2_p_track(i,0) = x2_p(i)
				p_nodes(i, 1) = x1_root
				p_nodes(i, 2) = x2_root
