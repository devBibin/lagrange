...
MODULE o_lagrange
	...
	integer(4) :: MAIN_PROC
	real(8) :: GRID_STEP, GRID_START_W, GRID_START_H
contains
	!-----------------MEMORY SECTION----------------------!
	! Checking allocation
	...
	SUBROUTINE o_fill_particles
		x1_p = 3 * GRID_STEP + GRID_START_W
		x2_p = 3 * GRID_STEP + GRID_START_H
			...
			do while (i <= n_p * (2*timesteps + 1))
				if (mod(i, 2*timesteps+1) == 1) THEN
					...
					if (collector(i) /= -1) THEN
						write (12, '(I10,A,f15.6,A,f15.6)'),ind,';',collector(i),';', collector(i+timesteps)
					end if
				end if
	...
	SUBROUTINE get_nearest_node_w(w, res)
		real(8), intent(in) :: w
		integer(4), intent(out) :: res
		res = FLOOR((w-GRID_START_W)/GRID_STEP)
	END SUBROUTINE

	SUBROUTINE get_nearest_node_h(h, res)
		real(8), intent(in) :: h
		integer(4), intent(out) :: res
		res = FLOOR((h-GRID_START_H)/GRID_STEP)
	END SUBROUTINE
	...
	SUBROUTINE o_update(step)
		...
		do n=1, n_p
			if (flags(n) == 1) then
				call get_nearest_node_w(x1_p(n), x1)
				call get_nearest_node_h(x2_p(n), y1)
				x2 = x1 + 1
				y2 = y1
				x3 = x1
				y3 = y1 + 1
				...
				call o_bilinear_interp(x1_p(n),x2_p(n),x1_v(x3,y3),x2_v(x3,y3),x1_v(x2,y2),x2_v(x2,y2),u(1),u(2),u(3),u(4),u_c_inter) 
		    		call o_bilinear_interp(x1_p(n),x2_p(n),x1_v(x3,y3),x2_v(x3,y3),x1_v(x2,y2),x2_v(x2,y2),v(1),v(2),v(3),v(4),v_c_inter)	
		
				x1_p_track(n,step) = x1_p(n)
				x2_p_track(n,step) = x2_p(n)
				
				x1_p(n) = x1_p(n) + (u_c_inter*dt*180.)/(radius*pi*cos(x2_p(n)/180.*pi))
				!x1_p(n) = x1_p(n) + (u_c_inter*dt*180.)/(radius*pi)
				...
				if (flags(n) == 0) then
					...
					if (subfield == -1) then
						...
						!print *, 'SEND TO ', subfield, 'Particle=', n, 'Iter=', step
